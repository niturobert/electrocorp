...
export default function Archive(props) {
    ...
    const [windowDimension, setWindowDimensions] = useState({width: 1920, height: 1080})
    ...
    async function updateRendering(force=false) {
        ...
        if (checkedState[0]) {
            if (powerPlants.length === 0 || force === true) {
                fetch("https://electrocorp-api.herokuapp.com/power_plant/all", {
                    method: 'POST',
                    cache: 'no-cache',
        ...
        if (checkedState[3]) {
            if (powerLines.length === 0 || force === true) {
                await fetch("https://electrocorp-api.herokuapp.com/power_line/all", {
                    cache: 'no-cache',
                    method: 'POST',
                    body: JSON.stringify({token: Cookies.get('token')})
                })
                ...
                <div className="grid grid-cols-3 w-full mr-8">
                    <main className="col-span-2 transition duration-500 ease-in-out overflow-y-auto">
                        <Globe
                            ...
                            width={(windowDimension.width - 96 ) * 0.75}
